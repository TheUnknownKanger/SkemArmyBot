...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import skembot.utils

        path = Path(f"skembot/plugins/{shortname}.py")
        name = "skembot.plugins.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = skembot.utils
        mod.Config = Config
        mod.borg = bot
        mod.edit_or_reply = edit_or_reply
        # support for paperplaneextended
        sys.modules["skembot.events"] = skembot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["skembot.plugins." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"skembot.plugins.{shortname}"
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/skemuserbot)"
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"skembot/plugins/assistant/{shortname}.py")
        name = "skembot.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["skembot.plugins.assistant" + shortname] = mod
